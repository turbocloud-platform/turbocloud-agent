...
func deployImage(image Image, job DeploymentJob, deployment Deployment) {
	...
	if service.ImageName != "" {
		...
		docker container run -p {{.MACHINE_VPN_IP}}:{{.MACHINE_PORT}}:{{.SERVICE_PORT}} -d -v {{.ENVIRONMENT_ID}}:/data --restart unless-stopped --log-driver=journald --name {{.DEPLOYMENT_ID}}.1 {{.IMAGE_NAME}}
`)
	} else {
		scriptTemplate = createTemplate("run_container", `
		#!/bin/sh
